...
**UX**, **factual accuracy**, and **alignment with exam-board (UK) exam standards**, with the
aural corpus as the centrepiece. Compiled 2026-07-03 from a full code review plus
verification against the current official exam-board syllabuses.

**Ground truth used** (stable through at least end of 2028; the 2025/26 practical-syllabus
revision changed repertoire only, and exam-board has confirmed no aural/supporting-test changes
for 2027/28 either):

- Aural tests: *exam-board Piano 2025 & 2026 Practical Grades Qualification Specification*,
  aural section pp. 45-52 (aural tests are identical across instruments).
  <https://www.example-syllabus-source.test/sites/default/files/2024-06/Piano%202025%20%26%202026%20Prac%20syllabus%2020240524_access.pdf>
- Theory: *exam-board Music Theory Syllabus from 2020*, Grades 1-5 and 6-8 sheets.
  <https://www.example-syllabus-source.test/sites/default/files/2023-09/music-theory-syllabus-outline-grades-1-5-from-2020.pdf>
  <https://www.example-syllabus-source.test/sites/default/files/2023-09/theory-syllabus-sheet-2020-g6-8.pdf>
...
**P1** = misaligned with exam-board or misleading. **P2** = quality/UX improvement.
**P3** = stretch.
...
2. **Grade-by-grade task inventory drifts from the real exam-board aural tests** in specific,
...
asking the exam-board-style question ("two, three, four or 6/8 time") rather than "how many
main beats".
...
### A2. Grade-by-grade alignment with the real exam-board aural tests (P1)
...
| Grade | exam-board test | App status | Action |
|-------|-----------|------------|--------|
...
| 2B | Echoes, **major only**, tonic-dominant | app adds minor phrases at G2 | **move minor echo phrases to G3**; exam-board introduces minor at 3B |
...
| 7C(ii) | **Name the two chords of the cadence just heard** - pool: tonic, subdominant, dominant, dominant 7th, submediant, all root position | app instead plays generic 3-chord progressions (I-IV-V, vi-IV-V...) including ii, which is not in the G7 pool | rework: after the cadence, ask which two chords; accept exam-board's three answer vocabularies (technical name / Roman numeral / letter name) |
...
| 8A(iii) | **Name all three chords incl. positions** of the cadential progression | app's "extended progressions" (e.g. vi-IV-V) are not cadential progressions from the exam-board set | rework to cadential 3-chord progressions from the G8 chord set, with position naming |
...
Terminology note: in the pulse tests exam-board asks for "two time / three time / four time /
...
`spec` captures exactly the exam-board constraints table above:
...
grade's rhythm palette instead of the fixed 0.5 s step. exam-board echo phrases have real
...
**A3.5 Cadence, chord and modulation stimuli in context.** exam-board plays a *phrase* whose
...
IV a, V a/b/c, V7 a, vi a). The G7 chord question then becomes exam-board's actual question:
...
question at G4+ (exam-board asks "describe the character": march-like, playful, songful,
...
- **A5.1 Tap the pulse (G1A's actual task).** Play a generated phrase; the learner taps
  ...
  scheme. This is the first test of every exam-board aural exam and the app currently has no
  version of it.
...
- **Grade 2 minor keys over-reach**: `src/content.js:1175` drills minors E, A, D, G, C.
  exam-board Grade 2 minors are **A, E, D only**; G minor (2♭) and C minor (3♭) belong at
...
- **Aural view framing text**: say "Practical exam" rather than "performance exam"
  (see ground-truth note above), and name the tests as exam-board does ("test 1A" style is
...
**Phase 2 - exam-board realignment (moderate).**
...
## Appendix: exam-board aural progression quick-reference
